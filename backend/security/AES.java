...
return result;
    ...
    public byte[] encrypt(byte[] input) {
        ...
        // Create result array for all blocks
        ...
        for (int blockStart = 0; blockStart < paddedInput.length; blockStart += 16) {
            ...
            // Load current block into state array
            for (int i = 0; i < 16; i++) {
                state[i % 4][i / 4] = paddedInput[blockStart + i];
            }
            ...
            for (int i = 0; i < 16; i++) {
                result[blockStart + i] = state[i % 4][i / 4];
            }
        }
    ...
    public byte[] decrypt(byte[] input) {
        ...
        if (input.length % 16 != 0) {
            throw new IllegalArgumentException("Input length must be multiple of 16 bytes");
        }

        byte[] result = new byte[input.length];

        // Decrypt each 16-byte block
        for (int blockStart = 0; blockStart < input.length; blockStart += 16) {
            byte[][] state = new byte[4][Nb];
            
            // Load current block into state array
            for (int i = 0; i < 16; i++) {
                state[i % 4][i / 4] = input[blockStart + i];
            }
